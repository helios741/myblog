...
client-go支持Godep和dep作为vendor的管理程序，我觉得dep便于使用所以继续使用dep。例如，一下是client-go v6.0和k8s API v1.9所需最低限度的`Gopkg.toml`。
...
### 从集群
当你的代码运行在这个集群中的时候，你可以用上面的函数并且不实用任何参数，这个函数就会通过集群的信息去连接api server。

`clientcmd.BuildConfigFromFlags("", "")`

或者我们可以通过rest包来创建一个使用集群中的信息去配置启动的，如下：
```
...
k8s的Go client框架支持为指定的API对象，在其生命周期事件中监听集群的能力，包括创建，更新，删除一个指定对象时候生成的`CREATED`,`MODIFIED`,`DELETED`。对于我们的命令行工具，我们将要监听在集群中已经声明的PVC的总量。
...
接下来我们将要处理资源事件。但是在我们处理事件之前，我们先声明`resource.Quantity`类型的的两个变量为`maxClaimsQuant `和`totalClaimQuant `来分别表示我们的申请资源阈值和运行总数。
...
代码也会在PVC被删除的时候做出反应，它执行相反的逻辑以及把被删除的这个PVC的申请的容量在正在运行的容量的总值里面减去。
